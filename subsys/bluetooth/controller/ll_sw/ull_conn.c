				      ...
				      uint32_t ticks_at_expire)
{
	...
	if (conn->llcp_cu.state != LLCP_CUI_STATE_INPROG) {
		...
		if (0) {
			...
			/*BLEX: We modified the Zephyr's controller code, because the original Zephyr code does not support anchor points change operation*/
		...
		if (!conn->lll.role) {
			...
			if ((interval_min < 6) ||
			    ...
			    ((timeout * 4U) <=
			     ((latency + 1) * interval_max)) ||
			    (preferred_periodicity > interval_max) ||
				cpr->offset0 == conn->llcp_conn_param.offset0) {
			...
			if ((conn->llcp_conn_param.interval_max !=
			    ...
			    (RADIO_CONN_EVENTS(conn->llcp_conn_param.timeout *
					       ...
					       1250) !=
			     conn->supervision_reload)||
(conn->llcp_conn_param.offset0 != 0 && offset < 4000 )) {
conn->llcp_conn_param.state =
					LLCP_CPR_STATE_APP_WAIT;
				old_offset = conn->llcp_conn_param.offset0;
			}
			 else {
				conn->llcp_conn_param.status = 0U;
				conn->llcp_conn_param.cmd = 0U;
